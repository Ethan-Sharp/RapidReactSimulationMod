...
function aim()
{
    if (isholding(LEFT)) {
        robot.xvel += -.2;
        robot.xvel = Math.max(robot.xvel, -4);
    } else if (isholding(RIGHT)) {
        robot.xvel += .2;
        robot.xvel = Math.min(robot.xvel, 4);
    } else {
        z = robot.xvel;
        if (robot.xvel > 0) {
            robot.xvel += -.2;
            robot.xvel = Math.max(robot.xvel, -4);
        } else if (robot.xvel < 0) {
            robot.xvel += .2;
            robot.xvel = Math.min(robot.xvel, 4);
        }
        if (Math.sign(z)==-Math.sign(robot.xvel)) {
            robot.xvel=0;
        }
    }
    ...
    if (robot.x < .8) {
        // robot.x = .4;
        robot.xvel += (.8-robot.x)*Math.abs(robot.xvel);
    }
    if (robot.x > 7.2) {
        // robot.x = 7.6;
        robot.xvel += (7.2-robot.x)*Math.abs(robot.xvel);
    }
    ...
    arrow.pointat(x+robot.xvel/5., y);
...
function startgame()
{
    ...
    robotlabel.x = robot.x+.245;
    robotlabel.y = robot.y+.05;
